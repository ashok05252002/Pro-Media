...
const PostCreation = () => {
    ...
    if (selectedItem && selectedItem.id) {
      extCompanyProductDataById(selectedItem.id).then((response) => {
        ...
        if (response.status === (200 || 201)) {
          ...
          const ids = data.map((item) => item.data_source_id);
          setAllDataSourceIds(ids); // ✅ trigger re-render
  ...
  const submitPost = async (status) => {
    ...
    try {
      ...
      const payload = {
        ext_product_data_source_id: selectedItem.id || "",
