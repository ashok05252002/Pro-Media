                ...
                <div key={post.video_id} className={`rounded-lg shadow-lg overflow-hidden transition-transform hover:scale-[1.02] ${post.color}`}>
                  <div className="p-6">
                    ...
                    <div className="mt-4 flex justify-between border-t pt-4 border-gray-200 dark:border-gray-700">
                      ...
                      <button onClick={() => handleViewPostComments(post?.id, post?.platformType)} className="flex items-center text-gray-600 dark:text-gray-400 hover:text-theme-primary dark:hover:text-theme-primary" title="View Comments"><MessageSquare className="w-4 h-4 mr-1" /><span className="text-sm">{ filteredComments.filter(comment => comment.product_data_source_video_id === post.id).length}</span></button>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg mb-6">
            <div className="p-4 border-b border-gray-200 dark:border-gray-700">
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                ...
                <div className="relative w-full lg:w-auto mt-4 lg:mt-0"><Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" size={18} /><input type="text" className="px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-theme-primary dark:bg-gray-700 text-sm pl-10 w-full lg:w-64" placeholder="Search comments..." value={searchQueryComments} onChange={(e) => setSearchQueryComments(e.target.value)} />{filteredComments.length}</div>
              </div>
