...
const PostCreation = () => {
  ...
  try {
    const response = await axios.post(
      'http://127.0.0.1:5001/post_system/addposts',
      posts,
      {
        headers: {
    ...
    alert('Posts published: ' + JSON.stringify(response.data));
  ...
  useEffect(() => {
  axios.get('http://127.0.0.1:5001/ext-product/list')
    .then(response => {
  ...
  useEffect(() => {
  if (selectedBusiness) {
    axios.get(`http://127.0.0.1:5001/ext-product/list_datasource/${selectedBusiness}`)
      .then(response => {
